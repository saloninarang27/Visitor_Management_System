...
const Visitors = ({ visitors, totalVisitors, isLoading, onActionClick, searchParams, setSearchParams, fetchData }) => {
// ✅ fetchData added to receive from props
...
if (!currentSelectedVisitor) return;

try {
const response = await fetch(`${url}/visitor/visitor-info/${currentSelectedVisitor.id}`, {
method: "DELETE",
headers: {
...
if (response.ok) {
Notification.showSuccessMessage("Success", "Visitor deleted successfully"); // ✅
setShowDeleteAlert(false);
fetchData(); // ✅ refresh list
} else {
const json = await response.json();
Notification.showErrorMessage("Error", json.error || "Failed to delete visitor"); // ✅
}
} catch (error) {
console.error("Error deleting visitor:", error);
Notification.showErrorMessage("Error", "Server error while deleting visitor"); // ✅
}
};
...
<input
className="appearance-none border border-blue-500 rounded-3xl w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-700"
type="text"
name="first_name"
...
className="border border-blue-500 rounded-3xl bg-white py-2 px-3 text-gray-700 focus:outline-none"
>
...
<button className="flex items-center bg-blue-500 hover:bg-blue-700 text-white py-1 px-4 rounded-3xl hover:scale-105 text-lg" onClick={() => { onActionClick('addNewVisitor'); handleClose(); }}>
...
<div className="bg-white shadow-md rounded my-6  mt-10">
<table className="text-left w-full border-collapse">
<thead>
<tr>
<th className="py-4 px-6 bg-blue-100 font-bold uppercase text-sm text-grey-dark border-b border-grey-light text-center">Visitor Image</th>
<th className="py-4 px-6 bg-blue-100 font-bold uppercase text-sm text-grey-dark border-b border-grey-light">
Visitor Name
</th>
<th className="py-4 px-6 bg-blue-100 font-bold uppercase text-sm text-grey-dark border-b border-grey-light">
Visitor Type
...
<tr key={visitor.id} className="hover:bg-grey-lighter hover:bg-blue-50" >
<td className="py-1 px-1 border-b border-grey-light">
<div className="flex justify-center">
<div className="inline-block h-16 w-16 border-2 border-white rounded-full overflow-hidden bg-customGreen">
{visitor.image ? (
<img src={`data:image/jpeg;base64,${visitor.image}`} alt="User" />
) : (
<div className="h-full w-full flex items-center justify-center text-black bg-customGreen text-md ">
